...
{
__global uint8_t* salt; // dkey, div, buf is temporary
    uint16_t itercount;
    __global uint8_t *crypto;
    long crypto_len;
...
struct npki_brute_force
{
	char* password;		// Password
   	 __global char* pw_charset;	// Password Charset
...
void memcpy_private_to_private(__private const char* src, __private char* dst, size_t size)
{
	int i;
	for(i=0; i<size; ++i)
	{
...
void memcpy_global_to_private(__global const char* src, __private char* dst, size_t size)
{
	int i;
	for(i=0; i<size; ++i)
	{
...
void memcpy_private_to_global(__private const char* src, __global char* dst, size_t size)
{
	int i;
	for(i=0; i<size; ++i)
	{
...
void memcpy_global_to_global(__global const char* src, __global char* dst, size_t size)
{
  int i;
  for(i=0; i<size; ++i)
  {
...
void memcpy(const char* src, char* dst, size_t size)
{
	int i;
	for(i=0; i<size; ++i)
	{
...
uint8_t* JV_PBKDF1(uint8_t* dkey, __private const uint8_t password[], const size_t pwlen, __global const uint8_t salt[], const size_t saltlen, const uint32_t itercount)
{
...
// First, cat [password] and [salt] into [immediate]
	...
	memcpy_private_to_global(kickoff + pwlen, salt, saltlen);
...
void JV_SEED_CBC128_Decrypt_Serial(__global const uint8_t *in, __global uint8_t *out, size_t len, __private const uint32_t *K, uint8_t ivec[SeedBlockSize])
{
	size_t n;
...
// Input, Output, Key, Vector중 NULL이 있는지 검사한다.
    ...
    {
        unsigned char c;
        uint32_t* dword_tmp = (uint32_t *)tmp;
// Do SEED decipher!
		uint32_t L0, L1, R0, R1;		// Iuput/output values at each rounds
		uint32_t T0, T1;				// Temporary variables for round function F
		// const uint32_t *K;			// Pointer of round keys

	// Set up input values for first round
		L0 = ((__global uint32_t *)in)[0];
		L1 = ((__global uint32_t *)in)[1];
		R0 = ((__global uint32_t *)in)[2];
		R1 = ((__global uint32_t *)in)[3];
...
void PasswordGenerate (NPKIBruteForce *bforce, char password[], __global uint64_t* max_cursor, uint32_t charset_len)
{
	uint32_t i = 0;
...
int IsPKCS5PaddingOK(__global const uint8_t* buf, const uint32_t buflen)
{
...
void NPKIDecrypt (NPKIPrivateKey *pkey, __private const char* password)
{
// 변수 선언 및 초기화
...
// Get SEED Key
    ...
    memcpy_private_to_private(seedkey, dkey, 16);
// Get SEED IV
    memcpy_private_to_private(buf, dkey+16, 4);
    JV_SHA1(div, buf, 4);
    memcpy_private_to_private(iv, div, 16);
	JV_SeedRoundKey(roundkey, seedkey);
	JV_SEED_CBC128_Decrypt_Serial(pkey->crypto, pkey->plain, pkey->crypto_len, roundkey, iv);
}
void bforcePkeyInit(  __global uint8_t* salt_pkey,
					uint16_t itercount_pkey,
					__global uint8_t *crypto_pkey,
					long crypto_len_pkey,
					__global uint8_t *plain_pkey,
					char* password_bforce,
...
{
	pkey->salt = salt_pkey;
	pkey->itercount = itercount_pkey;
	pkey->crypto = crypto_pkey;
	pkey->crypto_len = crypto_len_pkey;
	pkey->plain = plain_pkey;

	bforce->password = password_bforce;
	bforce->pw_charset = pw_charset_bforce;
	bforce->pw_min_len = pw_min_len_bforce;
	bforce->pw_max_len = pw_max_len_bforce;
	bforce->pw_cursor = pw_cursor_bforce;
}